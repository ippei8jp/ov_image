...
# ユーザ定義モジュール
from ssd_visualizer import SsdVisualizer
from ssd_frame_processor import SsdFrameProcessor
...
def build_argparser():
    ...
    # for SSD Detector
    sdetect = parser.add_argument_group('SSD')
    sdetect.add_argument('-m_ssd', metavar="MODEL_PATH", default="", required=True,
                        help="Path to the Face Detection model XML file")
    sdetect.add_argument('-d_ssd', default='CPU', choices=DEVICE_KINDS,
                       help="(optional) Target device for the " \
                       "Face Detection model (default: %(default)s)")
    sdetect.add_argument('-t_ssd', metavar='[0..1]', type=float, default=0.6,
                       help="(optional) Probability threshold for SSD" \
                       "(default: %(default)s)")
...
def main() :
    ...
    # Pre-process
    visualizer = SsdVisualizer(args)
    frame_processor = SsdFrameProcessor(args)
